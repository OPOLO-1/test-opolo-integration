...
void litecoinAddressConv(char * oldAdd, char * newAdd)
{
	char alphabet[58] = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
	int bytes[64];
	int byteLen = 0;
	bytes[byteLen++] = 0;
	int base = 58;
	char leader = alphabet[0];
	int carry1;
	int carry2;
	uint8_t digits[64];
	uint8_t payload[25];
	uint8_t digest[64];
	int stringLength = strlen(oldAdd);
	for(int i = 0; i< stringLength; i++)
	{
		uint8_t value = getAlpabetID(oldAdd[i]);
		carry1 = value;
		...
		while(carry1>0)
		{
			bytes[byteLen++] = carry1 & 0xff;
			carry1 >>= 8;
		}
	}
	for(int k = 0; oldAdd[k] == leader && k < (base - 1); ++k)
	{
		bytes[byteLen++] = 0;
	}
	int start = 0;
	...
	sha256_Raw(payload, 21, digest);
	sha256_Raw(digest, 32, digest);

	for(int i =0; i <4; i++){
		payload[21+i] = digest[i];
 	}
	int payloadLen = 25;
	int digitLen = 0;
	digits[digitLen++] = 0;
	for(int i =0; i <payloadLen; ++i){
		...
		for(int j = 0; j  < digitLen; ++j){
			carry2 += digits[j]<<8;
			digits[j] = carry2 % base;
			carry2 = (carry2/base);
		}
		while(carry2 >0){
			digits[digitLen++] = carry2%base;
			carry2 = carry2/base;
		}
	}
	int newAddLen = 0;
	for(int k = 0; payload[k] ==0 && k< payloadLen - 1; ++k){
		newAdd[newAddLen++] = leader;
	}
	for(int q =digitLen -1; q >=0; --q)
	{
		newAdd[newAddLen++] = alphabet[digits[q]%58];
	}
	newAdd[newAddLen++] = 0;
}
